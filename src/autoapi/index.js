...
class AutoAPI {
  ...
  route(method, path, spec, ...handlers) {
    ...
    if (path in this.paths === false) this.paths[path] = {};

    for (let hook in hooks) this.hooks[hook].call(this, fltSpec, hooks[hook]);
  ...
  static schemaFromModel(model, overrides = {}) {
    ...
    for (let key in model.schema.obj) {
      ...
      if (key in spec.properties) continue;
      ...
      if (apispec.example) spec.example[key] = apispec.example;
    }
