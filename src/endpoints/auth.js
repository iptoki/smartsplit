...
api.post(
	"/auth/login",
	{
		tags: ["Authorization"],
		summary:
			"Trades an email and password against an access token for use with the API.",
...
api.get(
	"/auth/check",
	{
		tags: ["Authorization"],
		summary:
			"Verifies an access token by returning `true` if it's valid, `false` otherwise.",
		hooks: { auth: false },
		responses: {
	...
	async function (req, res) {
		try {
			...
			if (e instanceof JWTAuth.Error) return false
			else throw e
		}
	}
)
