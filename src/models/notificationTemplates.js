...
const TemplateMap = {
			...
			generate: function (user, options) {
				...
				return {
					...
					data: {
						...
						callbackUrl: `${Config.clientUrl}/activate-additional-email/${token}`,
					},
				}
			},
		},
	...
	[UserTemplates.VERIFY_MOBILE_PHONE]: {
		...
		sms: {
			message: {
				en: 'Your Smartsplit activation code is {{code}}',
				fr: "Votre code d'activation Smartsplit est {{code}}",
			},
			generate: function (user) {
				return {
					message: this.message[user.locale].replace(
						'{{code}}',
						user.mobilePhone.verificationCode.code
					),
				}
			},
		},
